...
interface SalesPerformanceReportPreviewDialogProps {
  report: SalesPerformanceReport;
  templateId?: string;
  onClose: () => void;
}

export function SalesPerformanceReportPreviewDialog({
  report,
  templateId,
  onClose,
}: SalesPerformanceReportPreviewDialogProps) {
  const [preview, setPreview] = useState<{ renderedHtml: string; templateId?: string | null } | null>(null);
  ...
  useEffect(() => {
    const loadPreview = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const result = await salesPerformanceReportsApi.preview(report.id, { templateId });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="relative flex h-full max-h-[90vh] w-full max-w-5xl flex-col rounded-lg bg-card shadow-lg">
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            <h2 className="text-lg font-semibold text-foreground">Sales Performance Report Preview</h2>
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {!isLoading && !error && preview && (
            <iframe
              title="Sales Performance Report preview"
